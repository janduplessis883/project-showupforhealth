...
def make_global_disease_register(surgery_list = ['ECS', 'TCP', 'TGP', 'SMW', 'KMC', 'HPVM']):
    ...
    for surgery in surgery_list:
        ...
        full_register['distance_to_surg'] = full_register.apply(lambda row: haversine_distance(surgery, row['Latitude'], row['Longitude']), axis=1)
    ...
    print(f'✅ Global Disease Register Saved to output-data: global_disease_register.csv {global_register.shape}')
